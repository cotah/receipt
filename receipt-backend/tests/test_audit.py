...
class TestCatalogWorker:
    def test_save_tesco_catalog_basic(self):
        """Save a Tesco catalog item into collective_prices."""
        from app.workers.catalog_worker import save_tesco_catalog_items

        mock_db = MagicMock()
        mock_db.table.return_value.upsert.return_value.execute.return_value = MagicMock()

        items = [{
            "name": "Tesco Fresh Milk 2 Litre",
            "price": 2.25,
            "gtin": "05011100650236",
            "brand_name": "TESCO",
        ...
        result = save_tesco_catalog_items(mock_db, items)
        assert result["products_saved"] == 1
        assert result["errors"] == 0

    def test_save_tesco_catalog_skips_invalid(self):
        ...
        result = save_tesco_catalog_items(mock_db, items)
        assert result["products_saved"] == 0

    def test_save_tesco_catalog_multibuy_correction(self):
        """Correct multi-buy deal prices."""
        from app.workers.catalog_worker import save_tesco_catalog_items
        ...
        items = [{
            ...
            "promotion": "Any 2 for €6.50",
            "in_stock": True,
        }]

        result = save_tesco_catalog_items(mock_db, items)
        assert result["products_saved"] == 1
        # The function calls db.table("collective_prices").upsert(data, ...).execute()
        # Check the first positional arg of the first upsert call
        calls = mock_db.table.return_value.upsert.call_args_list
        assert len(calls) >= 1
        upserted = calls[0][0][0]  # First call, first positional arg
        assert upserted["unit_price"] == 3.25

    def test_map_category(self):
        """Category mapping works correctly."""
        from app.workers.catalog_worker import _map_category

        assert _map_category({"main_category": "Fresh Food"}) == "Fresh Food"
        assert _map_category({"main_category": "Bakery"}) == "Bakery"
        assert _map_category({"main_category": "Frozen Food"}) == "Frozen"
        assert _map_category({"main_category": "Drinks"}) == "Drinks"
        assert _map_category({"main_category": "", "sub_category": ""}) == "Other"

    def test_clean_gtin(self):
        """GTIN cleaning and validation."""
        from app.workers.catalog_worker import _clean_gtin

        assert _clean_gtin("05011100650236") == "5011100650236"
        assert _clean_gtin("00000003251416") == "03251416"  # Preserves 8 digits minimum
        assert _clean_gtin(None) is None
        assert _clean_gtin("") is None
        assert _clean_gtin("abc") is None
        assert _clean_gtin("123") is None  # Too short
